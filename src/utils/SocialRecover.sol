...
/// @dev
///  - Maintains a guardian set with proposal/revocation timelocks (`securityPeriod`) and an execution window
/// (`securityWindow`).
...
contract SocialRecoveryManager is EIP712, ISocialRecoveryManager {
    ...
    function initializeGuardians(address _account, bytes32 _initialGuardian) external override {
        ...
        IOPF7702Recoverable.GuardianIdentity storage gi = guardiansData[_account].data[_initialGuardian];
     ...
     */
    function startRecovery(address _account, IKey.KeyDataReg calldata _recoveryKey) external override {
        if (!isGuardian(_account, msg.sender.computeHash())) {
            revert IOPF7702Recoverable.OPF7702Recoverable__MustBeGuardian();
        }
        if (_recoveryKey.keyType == IKey.KeyType.P256 || _recoveryKey.keyType == IKey.KeyType.P256NONKEY) {
     ...
     */
    function getPendingStatusGuardians(address _account, bytes32 _guardian) external view override returns (uint256) {
    ...
    function _validateSignatures(address _account, bytes[] calldata _signatures) internal view returns (bool) {

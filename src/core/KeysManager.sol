...
abstract contract KeysManager is BaseOPF7702, IKeysManager, IKey {
     ...
     *      - `_keyData.limits > 0` (`mustHaveLimits()`), total transactions limit, this path does not register master
     * keys.
     ...
     */
    function setTokenSpend(bytes32 _keyId, address _token, uint256 _limit, SpendPeriod _period) public {
        _requireForExecute();
     ...
     */
    function updateTokenSpend(bytes32 _keyId, address _token, uint256 _limit, SpendPeriod _period) public {
        _requireForExecute();
        KeyData storage sKey = keys[_keyId];
        sKey.validateKeyBefore();
    ...
    function _addKey(KeyDataReg memory _keyData) internal {
        ...
        if (_keyData.keyControl == KeyControl.Custodial) {
            ...
            IUserOpPolicy(GAS_POLICY).initializeGasPolicy(address(this), keyId, uint256(_keyData.limits));
        }
     ...
     */
    function canExecuteAt(bytes32 _keyId, uint256 i) external view returns (address target, bytes4 fnSel) {
     ...
     */
    function hasCanCall(bytes32 _keyId, address _target, bytes4 _funSel) external view returns (bool) {
    ...
    function _masterKeyValidation(KeyDataReg memory _keyData) internal pure {
        _keyData.keyCantBeZero();
        if (
            _keyData.limits != 0 || _keyData.validAfter != 0 || _keyData.validUntil != type(uint48).max
                || _keyData.keyControl != KeyControl.Self || _keyData.keyType == KeyType.P256

...
contract OPF7702Recoverable is OPF7702, EIP712, ERC7201 {
        ...
        initializer
    {
        ...
        ISocialRecoveryManager(RECOVERY_MANAGER).initializeGuardians(address(this), _initialGuardian);
    ...
    function _setNewMasterKey(KeyDataReg memory recoveryOwner) private {
        ...
        emit KeyRegistered(keyId, recoveryOwner.keyControl, recoveryOwner.keyType, true, 0, type(uint48).max, 0);
        ...
        returns (bytes32 digest)
    {
        ...
        bytes32 structHash = keccak256(abi.encode(INIT_TYPEHASH, keyDataEnc, skDataEnc, _initialGuardian));

...
contract OPF7702 is Execution, Initializable {
     ...
     */
    function _isCanCall(bytes32 _keyId, address _target, bytes memory _data) internal view returns (bool) {
     ...
     * @param _keyId   Identifier of the key.
     * @param _target  Original call target (token address for ERC-20; replaced with `NATIVE_ADDRESS` for native
     * transfers).
     ...
     */
    function _manageTokenSpend(bytes32 _keyId, address _target, uint256 _tokenAmout) private returns (bool) {
    ...
    function startOfSpendPeriod(uint256 unixTimestamp, SpendPeriod period) public pure returns (uint256) {
        ...
        if (period == SpendPeriod.Day) return Math.rawMul(Math.rawDiv(unixTimestamp, 86_400), 86_400);

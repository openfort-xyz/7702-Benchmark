...
contract BenchmarksEOASessionKeyBatch10 is DeployAccount {
    ...
    function setUp() public override {
        ...
        _createCustomFreshKey(
            false, KeyType.EOA, uint48(block.timestamp + 1 days), 0, 1000, _getKeyEOA(sessionKey), KeyControl.Self
        );
        _initializeAccount();
    ...
    function test_SendBatch10CallWithEOASessionKeyDirectAA() external {
        ...
        _relayUserOp(userOp, "test_SendBatch10CallWithEOASessionKeyDirectAA");
    }

    function test_SendBatch10CallWithEOASessionKeyDirectAASponsored() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(10, address(erc20), 0, data);
        ...
        _relayUserOp(userOp, "test_SendBatch10CallWithEOASessionKeyDirectAASponsored");
    }

    function test_SendBatch10CallWithEOASessionKeyDirectAASponsoredERC20() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(10, address(erc20), 0, data);
        ...
        _relayUserOp(userOp, "test_SendBatch10CallWithEOASessionKeyDirectAASponsoredERC20");
    }

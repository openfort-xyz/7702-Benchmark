...
contract BenchmarksEOASessionKeyCustodialUniswapV2 is DeployAccount {
    ...
    function test_SendUniswapV2CallWithEOASessionKeyCustodialDirectAA() external {
        ...
        _relayUserOp(userOp, "test_SendUniswapV2CallWithEOASessionKeyCustodialDirectAA");
    }

    function test_SendUniswapV2CallWithEOASessionKeyCustodialDirectAASponsored() external {
        bytes memory data = _uniswapV2SwapPayload();
        Call[] memory calls = _getCalls(1, _UNISWAP_V2_ROUTER_ADDRESS, 0, data);
        ...
        _relayUserOp(userOp, "test_SendUniswapV2CallWithEOASessionKeyCustodialDirectAASponsored");
    }

    function test_SendUniswapV2CallWithEOASessionKeyCustodialDirectAASponsoredERC20() external {
        bytes memory data = _uniswapV2SwapPayload();
        Call[] memory calls = _getCalls(1, _UNISWAP_V2_ROUTER_ADDRESS, 0, data);
        ...
        _relayUserOp(userOp, "test_SendUniswapV2CallWithEOASessionKeyCustodialDirectAASponsoredERC20");
    }
    ...
    function _warmUpAccount()
        internal
        setTokenSpendM(KeyType.EOA, _getKeyEOA(sessionKey), token0, 1000 ether, IKeysManager.SpendPeriod.Month)
        setCanCallM(KeyType.EOA, _getKeyEOA(sessionKey), address(token0), ANY_FN_SEL, true)
        setCanCallM(KeyType.EOA, _getKeyEOA(sessionKey), address(_UNISWAP_V2_ROUTER_ADDRESS), ANY_FN_SEL, true)
    {
        _depositToPM();

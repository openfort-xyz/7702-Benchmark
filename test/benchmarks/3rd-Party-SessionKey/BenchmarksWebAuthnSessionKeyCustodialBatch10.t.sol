...
contract BenchmarksWebAuthnSessionKeyCustodialBatch10 is DeployAccount {
    ...
    function setUp() public override {
        ...
        _populateWebAuthn("WebAuthnSessionKey.json", ".batch10.DirectAA");
        pK_SK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });
        _createCustomFreshKey(
            false, KeyType.WEBAUTHN, uint48(block.timestamp + 1 days), 0, 1000, _getKeyP256(pK_SK), KeyControl.Custodial
        );
        _initializeAccount();
    ...
    function test_SendBatch10CallWithWebAuthnSessionKeyCustodialDirectAA() external {
        ...
        _relayUserOp(userOp, "test_SendBatch10CallWithWebAuthnSessionKeyCustodialDirectAA");
    }

    function test_SendBatch10CallWithWebAuthnSessionKeyCustodialDirectAASponsored() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(10, address(erc20), 0, data);
        ...
        _populateWebAuthn("WebAuthnSessionKey.json", ".batch10.AASponsored");
        pK_SK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });

        userOp.signature = _getSignedUserOpByWebAuthn(DEF_WEBAUTHN, pK_SK);

        _relayUserOp(userOp, "test_SendBatch10CallWithWebAuthnSessionKeyCustodialDirectAASponsored");
    }

    function test_SendBatch10CallWithWebAuthnSessionKeyCustodialDirectAASponsoredERC20() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(10, address(erc20), 0, data);
        ...
        _populateWebAuthn("WebAuthnSessionKey.json", ".batch10.AASponsoredERC20");
        pK_SK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });

        userOp.signature = _getSignedUserOpByWebAuthn(DEF_WEBAUTHN, pK_SK);

        _relayUserOp(userOp, "test_SendBatch10CallWithWebAuthnSessionKeyCustodialDirectAASponsoredERC20");
    }

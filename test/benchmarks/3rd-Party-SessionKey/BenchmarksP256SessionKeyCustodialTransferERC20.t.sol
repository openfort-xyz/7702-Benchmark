...
contract BenchmarksP256SessionKeyCustodialTransferERC20 is DeployAccount {
    ...
    function setUp() public override {
        ...
        _populateP256NON("P256SessionKey.json", ".transferERC20.DirectAA.result2");
    ...
    function test_SendTransferERC20CallWithP256SessionKeyCustodialDirectAA() external {
        ...
        _relayUserOp(userOp, "test_SendTransferERC20CallWithP256SessionKeyCustodialDirectAA");
    }

    function test_SendTransferERC20CallWithP256SessionKeyCustodialAASponsored() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(1, address(erc20), 0, data);
        ...
        _populateP256NON("P256SessionKey.json", ".transferERC20.AASponsored.result2");
        pK_SK = PubKey({ x: DEF_P256.X, y: DEF_P256.Y });

        userOp.signature = _encodeP256Signature(DEF_P256.R, DEF_P256.S, pK_SK, KeyType.P256NONKEY);

        _relayUserOp(userOp, "test_SendTransferERC20CallWithP256SessionKeyCustodialAASponsored");
    }

    function test_SendTransferERC20CallWithP256SessionKeyCustodialAASponsoredERC20() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(1, address(erc20), 0, data);
        ...
        _populateP256NON("P256SessionKey.json", ".transferERC20.AASponsoredERC20.result2");
        pK_SK = PubKey({ x: DEF_P256.X, y: DEF_P256.Y });

        userOp.signature = _encodeP256Signature(DEF_P256.R, DEF_P256.S, pK_SK, KeyType.P256NONKEY);

        _relayUserOp(userOp, "test_SendTransferERC20CallWithP256SessionKeyCustodialAASponsoredERC20");
    }
    ...
    function _warmUpAccount()
        internal
        setTokenSpendM(KeyType.P256NONKEY, _getKeyP256(pK_SK), address(erc20), 1000 ether, IKeysManager.SpendPeriod.Month)

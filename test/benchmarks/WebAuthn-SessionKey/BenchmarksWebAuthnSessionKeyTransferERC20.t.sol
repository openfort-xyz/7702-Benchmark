...
contract BenchmarksWebAuthnSessionKeyTransferERC20 is DeployAccount {
    ...
    function setUp() public override {
        ...
        _populateWebAuthn("WebAuthnSessionKey.json", ".transferERC20.DirectAA");
    ...
    function test_SendTransferERC20CallWithWebAuthnSessionKeyDirectAA() external {
        ...
        _relayUserOp(userOp, "test_SendTransferERC20CallWithWebAuthnSessionKeyDirectAA");
    }

    function test_SendTransferERC20CallWithWebAuthnSessionKeyDirectAASponsored() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(1, address(erc20), 0, data);
        ...
        _populateWebAuthn("WebAuthnSessionKey.json", ".transferERC20.AASponsored");
        pK_SK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });

        userOp.signature = _getSignedUserOpByWebAuthn(DEF_WEBAUTHN, pK_SK);

        _relayUserOp(userOp, "test_SendTransferERC20CallWithWebAuthnSessionKeyDirectAASponsored");
    }

    function test_SendTransferERC20CallWithWebAuthnSessionKeyDirectAASponsoredERC20() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(1, address(erc20), 0, data);
        ...
        _populateWebAuthn("WebAuthnSessionKey.json", ".transferERC20.AASponsoredERC20");
        pK_SK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });

        userOp.signature = _getSignedUserOpByWebAuthn(DEF_WEBAUTHN, pK_SK);

        _relayUserOp(userOp, "test_SendTransferERC20CallWithWebAuthnSessionKeyDirectAASponsoredERC20");
    }
    ...
    function _warmUpAccount()
        internal
        setTokenSpendM(KeyType.WEBAUTHN, _getKeyP256(pK_SK), address(erc20), 1000 ether, IKeysManager.SpendPeriod.Month)

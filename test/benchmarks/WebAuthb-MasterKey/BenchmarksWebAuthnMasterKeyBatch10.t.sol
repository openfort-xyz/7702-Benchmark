...
contract BenchmarksWebAuthnMasterKeyBatch10 is DeployAccount {
    ...
    function setUp() public override {
        ...
        _populateWebAuthn("WebAuthnMasterKey.json", ".batch10.DirectAA");
    ...
    function test_SendBatch10CallWithWebAuthnMasterKeyDirectAA() external {
        ...
        _relayUserOp(userOp, "test_SendBatch10CallWithWebAuthnMasterKeyDirectAA");
    }

    function test_SendBatch10CallWithWebAuthnMasterKeyDirectAASponsored() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(10, address(erc20), 0, data);
        ...
        _populateWebAuthn("WebAuthnMasterKey.json", ".batch10.AASponsored");
        pK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });

        userOp.signature = _getSignedUserOpByWebAuthn(DEF_WEBAUTHN, pK);

        _relayUserOp(userOp, "test_SendBatch10CallWithWebAuthnMasterKeyDirectAASponsored");
    }

    function test_SendBatch10CallWithWebAuthnMasterKeyDirectAASponsoredERC20() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(10, address(erc20), 0, data);
        ...
        _populateWebAuthn("WebAuthnMasterKey.json", ".batch10.AASponsoredERC20");
        pK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });

        userOp.signature = _getSignedUserOpByWebAuthn(DEF_WEBAUTHN, pK);

        _relayUserOp(userOp, "test_SendBatch10CallWithWebAuthnMasterKeyDirectAASponsoredERC20");
    }

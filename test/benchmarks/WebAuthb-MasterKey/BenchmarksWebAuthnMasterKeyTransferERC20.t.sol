...
contract BenchmarksWebAuthnMasterKeyTransferERC20 is DeployAccount {
    ...
    function setUp() public override {
        ...
        _populateWebAuthn("WebAuthnMasterKey.json", ".transferERC20.DirectAA");
    ...
    function test_SendTransferERC20CallWithWebAuthnMasterKeyDirectAA() external {
        ...
        _relayUserOp(userOp, "test_SendTransferERC20CallWithWebAuthnMasterKeyDirectAA");
    }

    function test_SendTransferERC20CallWithWebAuthnMasterKeyDirectAASponsored() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(1, address(erc20), 0, data);
        ...
        _populateWebAuthn("WebAuthnMasterKey.json", ".transferERC20.AASponsored");
        pK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });

        userOp.signature = _getSignedUserOpByWebAuthn(DEF_WEBAUTHN, pK);

        _relayUserOp(userOp, "test_SendTransferERC20CallWithWebAuthnMasterKeyDirectAASponsored");
    }

    function test_SendTransferERC20CallWithWebAuthnMasterKeyDirectAASponsoredERC20() external {
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, reciver, 10e18);
        Call[] memory calls = _getCalls(1, address(erc20), 0, data);
        ...
        _populateWebAuthn("WebAuthnMasterKey.json", ".transferERC20.AASponsoredERC20");
        pK = PubKey({ x: DEF_WEBAUTHN.X, y: DEF_WEBAUTHN.Y });

        userOp.signature = _getSignedUserOpByWebAuthn(DEF_WEBAUTHN, pK);

        _relayUserOp(userOp, "test_SendTransferERC20CallWithWebAuthnMasterKeyDirectAASponsoredERC20");
    }

...
library PimlicoHelpers {
    ...
    function getHashV7(uint8 _mode, PackedUserOperation calldata _userOp) public view returns (bytes32) {
        if (_mode == VERIFYING_MODE) {
            return _getHashV7(_userOp, MODE_AND_ALLOW_ALL_BUNDLERS_LENGTH + VERIFYING_PAYMASTER_DATA_LENGTH);
        } else {
    ...
    function getHashV6(uint8 _mode, UserOperation calldata _userOp) public view returns (bytes32) {
        if (_mode == VERIFYING_MODE) {
            return _getHashV6(_userOp, VERIFYING_PAYMASTER_DATA_LENGTH + MODE_AND_ALLOW_ALL_BUNDLERS_LENGTH);
        } else {
            uint8 paymasterDataLength = ERC20_PAYMASTER_DATA_LENGTH + MODE_AND_ALLOW_ALL_BUNDLERS_LENGTH;

            uint8 combinedByte = uint8(_userOp.paymasterAndData[20 + MODE_AND_ALLOW_ALL_BUNDLERS_LENGTH]);
            // constantFeePresent is in the *lowest* bit
            bool constantFeePresent = (combinedByte & 0x01) != 0;
            // recipientPresent is in the second lowest bit
            bool recipientPresent = (combinedByte & 0x02) != 0;
    ...
    function _getHashV6(UserOperation calldata _userOp, uint256 paymasterDataLength) internal view returns (bytes32) {
        bytes32 userOpHash = keccak256(
            abi.encode(
                _userOp.sender,
                _userOp.nonce,
...
contract MockOracle {
    ...
    function latestRoundData() external view returns (uint256, int256 answer, uint256, uint256 updatedAt) {
